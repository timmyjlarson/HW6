...
public class ColoredGraph {
    ...
    public Integer coloredMaze(int start, int end) {
        //YOUR CODE HERE
        Integer minLength = null;    //the minimum length from start to end
        Integer currentLength;        //the current length to the end using the specified edge
        GraphNode startNode = findNode(start);
        GraphNode endNode = findNode(end);
        if (startNode != endNode) {
            for (Edge edge : startNode.edges) {        //this loops through all of the edges at this node
                if (edge.color == "red" && startNode == edge.u && !edge.visited) {        //if the edge has not been transversed yet and it is the correct color
                    currentLength = yellowTransversal(end, edge.v, endNode);        //recurse on the next node
                    if (currentLength != null) {
                        currentLength++;        //when continuing up the tree, add one for each edge
                    }
                    if (minLength == null) {
                        minLength = currentLength;
                    } else if (currentLength != null && minLength > currentLength) {    //keep the smallest nonnull value
                        minLength = currentLength;
                    }
                } else if (edge.color == "red" && startNode == edge.v && !edge.visited) {    //if the edge has not been transversed yet and it is the correct color
                    currentLength = yellowTransversal(end, edge.u, endNode);        //recurse on the next node
                    if (currentLength != null) {
                        currentLength++;        //when continuing up the tree, add one for each edge
                    }
                    if (minLength == null) {
    ...
    private Integer redTransversal(int end, GraphNode startNode, GraphNode endNode) {
        ...
        if (startNode != endNode) {
            for (Edge edge : startNode.edges) {
                ...
                } else if (edge.color == "red" && startNode == edge.v) {        //if the edge has not been transversed yet and it is the correct color
                    currentLength = yellowTransversal(end, edge.u, endNode);    //recurse on the next node
                    if (currentLength != null) {
                        currentLength++;    //when continuing up the tree, add one for each edge
                    }
            ...
            return 0;    //if this node is the end node push 0 back up the tree
        }
        return minLength;
    }
    ...
    private Integer yellowTransversal(int end, GraphNode startNode, GraphNode endNode) {
        ...
        if (startNode != endNode) {
            for (Edge edge : startNode.edges) {
                if (edge.color == "yellow" && startNode == edge.u && !edge.visited) {    //if the edge has not been transversed yet and it is the correct color
                    currentLength = blueTransversal(end, edge.v, endNode);        //recurse on the next node
                    if (currentLength != null) {
                        currentLength++;        //when continuing up the tree, add one for each edge
                    }
                    if (minLength == null) {
                ...
                } else if (edge.color == "yellow" && startNode == edge.v && !edge.visited) {    //if the edge has not been transversed yet and it is the correct color
                    currentLength = blueTransversal(end, edge.u, endNode);        //recurse on the next node
                    if (currentLength != null) {
                        currentLength++;        //when continuing up the tree, add one for each edge
                    }
                    if (minLength == null) {
    ...
    private Integer blueTransversal(int end, GraphNode startNode, GraphNode endNode) {
        ...
        if (startNode != endNode) {
            for (Edge edge : startNode.edges) {
                if (edge.color == "blue" && startNode == edge.u && !edge.visited) {        //if the edge has not been transversed yet and it is the correct color
                    currentLength = redTransversal(end, edge.v, endNode);        //recurse on the next node
                    if (currentLength != null) {
                        currentLength++;        //when continuing up the tree, add one for each edge
                    }
                    if (minLength == null) {
                ...
                } else if (edge.color == "blue" && startNode == edge.v && !edge.visited) {    //if the edge has not been transversed yet and it is the correct color
                    currentLength = redTransversal(end, edge.u, endNode);        //recurse on the next node
                    if (currentLength != null) {
                        currentLength++;        //when continuing up the tree, add one for each edge
                    }
                    if (minLength == null) {
